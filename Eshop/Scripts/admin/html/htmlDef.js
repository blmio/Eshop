...
var Bx = {
    ...
    Submitlist: function () {
        ...
        if (leng == 0) pWin.Boxalert("请选择要生成的页面！");
        ...
        emed.each(function () {
            if ($(this).attr("evm") != 1) {
                ...
                if (mn == 1)
                    Bx.loading("正在生成" + thtml + "页");
                else
                    Bx.TanCstr("正在生成" + thtml + "页");
                var Vrr = val.split("|$|");
                var hurl = pWin.url + 'admin/Apply/HtmlWrite.aspx?mod=addhtml&D=' + escape(Vrr[0]);
