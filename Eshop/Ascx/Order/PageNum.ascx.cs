...
public partial class Ascx_Order_PageNum : System.Web.UI.UserControl
{
    #region  传递的参数
    ...
    public string input(string N)
    {
        if (pageinput == 0)
        {
            ...
            str += "<span>共 " + pageall + " 页</span>";
            str += "<span>到第 <input id='pageinp' type=\"text\" title=\"输入页码，按回车快速跳转\" value=\"" + pageint + "\" onkeydown=\"checkPage(event,this.value)\" /> 页</span>";
            str += "<button onClick=\"LPage(EDE.$('pageinp').value)\">确定</button>";
            return str;
        }
        else
            return "";
    ...
    {
        return ask.Web.AppInf.strid(p, url);
    }
}
